...
class VL53L1X:
    ...
    def int_status(self):
        if self._read_register(_RESULT__INTERRUPT_STATUS)[0] == 0x01:
            return True  # 0x01 for Thresh Low detection
        else:
            return False
    ...
    #     self._write_register(0x74, b"\x06\xD6")

#    def get_threshold_high(self):
#        status = self._read_register(SYSTEM__THRESH_HIGH)[0]
#        return status

#    def set_threshold_high(self, dist):
#        data = dist.to_bytes(2, byteorder='big')
#        self._write_register(SYSTEM__INTERRUPT_CONFIG_GPIO, b"\x00")
#        self._write_register(SYSTEM__THRESH_HIGH, data)

#    def get_threshold_low(self):
#        status = self._read_register(SYSTEM__THRESH_LOW)[0]
#        return status

#    def set_threshold_low(self, dist):
#        data = dist.to_bytes(2, byteorder='big')
#        self._write_register(SYSTEM__INTERRUPT_CONFIG_GPIO, b"\x00")
#        self._write_register(SYSTEM__THRESH_LOW, data)
